...
const InventorySection: React.FC<InventorySectionProps> = ({ onAddToCart, onSelectProduct }) => {
  ...
  return (
    <section id="inventory" className="py-32 px-6 md:px-12 max-w-[1800px] mx-auto min-h-screen">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-12 mb-20 border-b border-[#D1D1D1] pb-16">
        <div className="animate-fade-in-down">
          <h2 className="text-[#FF3E1F] uppercase tracking-[0.4em] text-[10px] font-black mb-6 italic">Rental Catalog</h2>
          <h3 className="text-5xl md:text-7xl font-heading font-black tracking-tighter text-[#121212] uppercase leading-[0.85]">
            ELITE GEAR.
          </h3>
        </div>
        ...
        <div className="flex flex-col gap-4 min-w-[340px]">
          ...
          <div className="relative">
            <input
              ...
              className="w-full bg-white border border-[#D1D1D1] px-8 py-5 text-[11px] font-bold focus:outline-none focus:border-[#FF3E1F] transition-all placeholder:text-black/20 text-[#121212] uppercase tracking-widest"
            />
          </div>
        </div>
      </div>
          ...
          {categories.map(cat => (
            <button
              ...
              className={`px-10 py-5 rounded-none text-[10px] uppercase font-black tracking-[0.3em] transition-all duration-500 ${activeCategory === cat
                ? 'bg-[#121212] text-white shadow-[0_20px_40px_-10px_rgba(0,0,0,0.3)] scale-105'
                : 'bg-white border border-[#D1D1D1] text-black/40 hover:border-black/60 hover:text-black'
                }`}
            >
              {cat}
            </button>
            ...
            {subcategories.map(sub => (
              <button
                ...
                className={`px-8 py-3 rounded-none text-[9px] uppercase font-black tracking-[0.2em] transition-all ${activeSubcategory === sub
                  ? 'bg-[#FF3E1F] text-white shadow-lg'
                  : 'bg-white border border-[#D1D1D1] text-black/20 hover:border-black/40 hover:text-black'
                  }`}
              >
                {sub}
              </button>
      ...
      {paginatedProducts.length > 0 ? (
            ...
            >
              ...
              >
                ...
                <div className="absolute top-6 left-6 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500">
                  <span className="bg-[#FF3E1F] text-white px-4 py-1.5 text-[9px] font-black uppercase tracking-widest italic shadow-xl">
                    {product.brand}
                  </span>
                </div>
              </div>
              ...
              <div className="p-8 flex flex-col flex-grow">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className={`text-[8px] font-black px-2 py-0.5 rounded-full border border-black/10 uppercase tracking-widest ${product.available > 0 ? 'text-green-600' : 'text-[#FF3E1F]'
                    }`}>
                ...
                <h3
                  className="text-2xl font-heading text-[#121212] font-black tracking-tight mb-8 cursor-pointer hover:text-[#FF3E1F] transition-colors leading-none uppercase"
                  onClick={() => onSelectProduct(product)}
                >
                  {product.name}
                </h3>
                ...
                <div className="flex justify-between items-end border-t border-[#F0F0F0] pt-6 mt-auto">
                  <div>
                    <span className="text-[#FF3E1F] font-black text-3xl tracking-tighter italic leading-none">${product.pricePerDay}</span>
                    <span className="text-[9px] text-black/20 ml-1 font-black uppercase tracking-widest block mt-1">per day</span>
                  </div>

                  <button
                    onClick={() => onAddToCart(product)}
                    className="h-11 px-8 bg-[#121212] text-white flex gap-3 items-center justify-center hover:bg-[#FF3E1F] transition-all transform active:scale-95 shadow-xl group/btn text-[10px] font-black uppercase tracking-[0.2em] italic"
                  >
                    ADD
        ...
        <div className="py-40 text-center border border-[#D1D1D1] bg-white animate-fade-in">
      ...
      {totalPages > 1 && (
        <div className="mt-24 flex items-center justify-center gap-6">
          <button
            ...
            className="w-16 h-16 border-2 border-[#D1D1D1] rounded-full flex items-center justify-center disabled:opacity-20 hover:border-[#FF3E1F] hover:text-[#FF3E1F] transition-all group bg-white shadow-lg"
          >
            ...
            {[...Array(totalPages)].map((_, i) => (
              <button
                ...
                className={`w-16 h-16 border-2 rounded-full flex items-center justify-center text-[11px] font-black tracking-widest transition-all ${currentPage === i + 1
                  ? 'border-[#FF3E1F] bg-[#FF3E1F] text-white shadow-2xl scale-110'
                  : 'border-[#D1D1D1] text-black/30 hover:border-black hover:text-black bg-white'
                  }`}
              >
                0{i + 1}
              </button>
